...
let
  ...
  desktop = cfg.desktop;
  confFile = pkgs.writeText (if desktop then "nohang-desktop.conf" else "nohang.conf") ''
    ${cfg.extraConfig}
  '';
...
in
{
  options.services.nohang = {
    enable = mkEnableOption (mdDoc "nohang");
    desktop = mkEnableOption (mdDoc "Whether GUI notification support should be enabled.");

    package = mkOption {
      type = types.package;
      default = pkg;
      description = "Package (derivation) that provides the nohang binary";
    };
