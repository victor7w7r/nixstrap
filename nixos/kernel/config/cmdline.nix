...
let
  ...
  opt = "mitigations=off pti=on page_alloc.shuffle=1 elevator=none";
  sec = "lsm=lockdown,yama,integrity,apparmor,bpf page_poison=1";
  ...
  rescue = "sysrq_always_enabled=1 udev.log_level=3 verbose=1";
  sata =
    if host == "v7w7r-youyeetoox1" || host == "v7w7r-macmini81" then "libahci.ignore_sss=1" else "";
  amd =
    if host == "v7w7r-rc71l" then
      "nospectre_v1 nospectre_v2 spec_store_bypass_disable=off amd_iommu=on amdgpu.sg_display=0"
    else
      "";
  intel =
    if host != "v7w7r-rc71l" then
  ...
  higole =
    if host == "v7w7r-higole" then
      ...
      "i915.enable_psr=0 acpi_backlight=vendor mem_sleep_default=deep i2c_dw.disable_fast_mode=1 i915.enable_dpcd_backlight=1";
