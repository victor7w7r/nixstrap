{ host, lib, pkgs, helpers, kernelData, hardened ? false, ... }:
let
  ...
  patches = let rmRandstruct = with lib; filter (p: !hasInfix "randstruct" p);
  in (rmRandstruct kernel.patches) ++ [
...
in pkgs.stdenv.mkDerivation (attrs: {
  ...
  nativeBuildInputs = with pkgs;
    kernel.nativeBuildInputs ++ kernel.buildInputs
    ++ [ clang_18 llvm_18 lld_18 ];
