...
}:
{
  ...
  system.build.installBootLoader = pkgs.writeScript "installBootLoader.sh" ''
    ...
    mkdir -p /boot/EFI/tools /boot/EFI/BOOT /boot/EFI/refind/themes

    cp ${pkgs.refind}/share/refind/refind_x64.efi /boot/EFI/refind/refind_x64.efi
    cp ${pkgs.refind}/share/refind/refind_x64.efi /boot/EFI/BOOT/BOOTX64.efi
    ...
    EFI_INFO=$(lsblk -o NAME,PARTTYPE,PKNAME,PARTTYPENAME,FSTYPE | grep -i "EFI" | grep -i "vfat" | head -n1)
