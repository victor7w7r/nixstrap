{ lib, ... }:
{
  den.default.provides.to-users.homeManager.programs =
    let
      ...
      equals = "realpath $(which $1)";
      mkcd = ''mkdir -p "$1" && cd "$1"'';
      dus = ''command dust -r -d 1 "$@"'';
      tka = "tmux ls | cut -d : -f 1 | xargs -I {} tmux kill-session -t {}";
      tna = ''tmux new-session -As "$(basename "$PWD" | tr . -)"'';
      bin = ''ln -sr "$(realpath "$1")" /usr/local/bin/'';
      path_add = ''
        if [[ ":$PATH:" != *":$1:"* ]]; then
          export PATH="$PATH:$1"
        fi
